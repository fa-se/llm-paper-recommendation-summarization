...
def add_topic(user_name: str, topic_id: int, relevance_score: float) -> None:
    ...
    with Session() as session:
        ...
        association_check = (
            session.query(UserTopicAssociation)
        ...
        if association_check is not None:
            typer.echo(
                f'User {user_name} already follows topic "{topic.name}" [{topic_id}].'
            )
            raise typer.Exit(code=-1)
...
def remove_topic(user_name: str, topic_id: int) -> None:
    with Session() as session:
        ...
        session.commit()
        typer.echo(
            f"Successfully removed topic {topic_id} from {user_name}'s followed topics."
        )
...
def set_area_of_interest(
    ...
    align = (
        not dont_align
    )  # Flip the value, because the feature should be enabled by default
    ...
    with Session() as session:
        ...
        if align:
            typer.echo(
                "Rewriting the area of interest description to improve similarity search..."
            )
        topics, scores, description = get_topics_from_description(
            session, llm_interface, area_of_interest_description, align=align
        )
        if align:
        ...
        typer.echo()
        user.config.area_of_interest_description = description
        user.config.followed_topics = [
            UserTopicAssociation(topic=topic, relevance_score=score)
            for topic, score in zip(topics, scores)
        ]

        session.commit()
        typer.echo(
            f"Successfully added these topics to the list of topics followed by user {user_name}."
        )
        typer.echo(
...
def get_topics_from_description(
    session: Session, llm_interface: LLMInterface, description: str, align: bool = True
    ...
    statement = (
        select(
            Topic,
            (1 - Topic.embedding.cosine_distance(query_embedding)).label(
                "cosine_similarity"
            ),
        )
